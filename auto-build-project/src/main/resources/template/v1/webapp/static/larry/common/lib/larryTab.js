...
layui.define(['jquery', 'larryElem', 'layer', 'common'], function(exports) {
  ...
  // 版本号
  LarryTab.prototype.version = '1.0';
  LarryTab.prototype.copyright = 'www.dongao.com';
  // 菜单独立设置
  ...
  LarryTab.prototype.init = function() {
    ...
    if (that.config.tabSession) {
      ...
      that.session(function(session) {
        // tabMenu是否存在
        if (session.getItem('tabMenu') && session.getItem('currentTabMenu') != 'undefined') {
          //若存在则还原tabMenu
  ...
  LarryTab.prototype.menu = function() {
    ...
    if (_config.data === undefined && _config.url === undefined) {
      common.larryCmsError('错误提示: 请为菜单项配置数据源【Data or Url】!', common.larryCore.errorDataTit);
    }
    if (_config.data !== undefined && typeof(_config.data) === 'object') {
      ...
      $.ajax({
        ...
        error: function(xhr, status, error) {
          common.larryCmsError('错误提示: ' + error, common.larryCore.errorDataTit);
        },
        complete: function() {
          _that.init();
        }
    ...
    if (res.eventName === 'click' && res.filter === _config.leftFilter) {
      if (_config.left_menu.attr('lay-filter') !== undefined) {
        ...
        _config.left_menu.find('li').each(function() {
          ...
          if ($this.hasClass('layui-this') && $this.find('cite').html() == '基础信息') {
              $this.find('a').eq(0).click();
              $this.find('.layui-nav-child').find('.layui-this a').click();
          }
  ...
  LarryTab.prototype.changeTab = function(id) {
    ...
    if (that.config.tabSession) {
      that.session(function(session) {
        if (session.getItem('currentTabMenu')) return false;
        var currentTabMenu = JSON.parse(session.getItem('currentTabMenu'));
  ...
  function getHtml(data, topStatus) {
    ...
    if (topStatus == 'on') {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        ulHtml.top += '<a  data-group="' + i + '" onmouseover="mhover()" >';
