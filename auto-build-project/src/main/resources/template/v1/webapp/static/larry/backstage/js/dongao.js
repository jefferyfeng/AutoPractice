...
layui.use(['jquery','larryElem','layer','common','form','larryMenu','larryTab'],function(){
    ...
    // 菜单初始化
    // 方法1：datas/data.json per/getMenu
	larryTab.menuSet({
		  tyep:'GET',
		  url: '../per/getMenu?t='+Math.random(),
		  topFilter: 'TopMenu',
		  lefFilter: 'LarrySide'
	});
    larryTab.menu();
  ...
  /*  $("#refresh_iframe").click(function() {
	...
	// 刷新iframe
	$("#refresh_iframe").click(function() {
		var flag = localStorage.getItem('autoRefresh');
        larryTab.tabSet({
            autoRefresh : true
        });
        $('.layui-nav-child .layui-this').click();
        if(flag == "true"){
            larryTab.tabSet({
                autoRefresh : true
            });
    ...
    var larryMenuData = [
		...
		[{
			text: "前往官网",
			func: function() {
				window.open('http://www.dongao.com');
			}
		}]
	];
