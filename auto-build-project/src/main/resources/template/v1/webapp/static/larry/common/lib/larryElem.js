  ...
  ,call = {
    ...
    ,clickThis: function(){
      ...
      if(!(elemA.attr('href') !== 'javascript:;' && elemA.attr('target') === '_blank')){
        ...
        if (elemA.attr('id') == 'logout' || elemA.attr('id') == 'clearCached' || elemA.attr('id') == 'FullScreen' ) {

        } else {
            /*if ($('.sys-menu-box .layui-nav-item').eq(0).find('a cite').html() == "首页") {
               // debugger;
               // $('.sys-menu-box .layui-nav-item').eq(0).find('a').eq(0).click();
            } else {

            }*/
            $('.layui-nav-itemed').find('.layui-nav-child').find('.layui-this a').click();
        }
      }
